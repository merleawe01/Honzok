...
AS
SELECT B.POST_NO, B.POST_TITLE, B.RECO_COUNT, B.CONTENT, B.VIEW_COUNT, E.RC_FOOD, E.ADDRESS, E.STAR, E.AREA_X, E.AREA_Y, M.NICKNAME
FROM B_COMMON B
JOIN EAT E ON (B.POST_NO = E.POST_NO)
JOIN MEMBER M ON (B.WRITER = M.USER_ID)
WHERE B.DELETE_YN = 'N';
...
AS
SELECT B.POST_NO, B.POST_TITLE, B.RECO_COUNT, B.CONTENT, B.VIEW_COUNT, T.CAUTION, T.BEST_TIME, T.ADDRESS, T.STAR, T.AREA_X, T.AREA_Y, M.NICKNAME
FROM B_COMMON B
JOIN TRIP T ON (B.POST_NO = T.POST_NO)
JOIN MEMBER M ON (B.WRITER = M.USER_ID)
WHERE B.DELETE_YN = 'N';
