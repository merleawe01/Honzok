...
<body onresize="parent.resizeTo(530,600)" onload="parent.resizeTo(530,600)">
	...
	<div class="tableArea">
	<form action="/list.re" id="RListForm" method="get">
		<table id="list">
			<tr>
				<th width=30px>
		...
		<div class="pagingArea" align="center">
			<% if(!list.isEmpty()) { %>
			<!-- 맨 처음으로 -->
			<button type="button" class="pagingBtn" onclick="location.href='<%= request.getContextPath()%>/list.re?currentPage=1'">&lt;&lt;</button>
			<!-- 버튼을 클릭하면 Servlet으로 넘어가서 currentPage가 1로 바뀐다. -->
				
			<!-- 이전 페이지로 -->
			<button type="button" onclick="location.href='<%= request.getContextPath()%>/list.re?currentPage=<%= currentPage - 1%>'"
			...
			<% for(int p = startPage; p <= endPage; p++){ %>
				...
				<% } else { %>
					<button type="button" id="numBtn" onclick ="location.href='<%= request.getContextPath()%>/list.re?currentPage=<%= p %>'"><%= p %></button>
				<% } %> 
			<% } %>
			
			<!-- 다음 페이지로  -->
			<button type="button" class="pagingBtn" onclick="location.href='<%= request.getContextPath() %>/list.re?currentPage=<%= currentPage + 1 %>'" id="afterBtn">next</button>
			<script>
			...
			<!-- 맨 끝으로 -->
			<button type="button" class="pagingBtn" onclick="location.href='<%= request.getContextPath()%>/list.re?currentPage=<%= maxPage %>'">&gt;&gt;</button>
